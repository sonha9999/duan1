...
export function useLandingPage() {
  ...
  const [recruitForm, setRecruitForm] = useState({ name: "", phone: "", position: "", experience: "" });
  ...
  const [contactForm, setContactForm] = useState({ name: "", phone: "", email: "", service: "", area: "", address: "", note: "" });
  const [contactLoading, setContactLoading] = useState(false);
  const [contactSuccess, setContactSuccess] = useState(false);

  // --- LOGIC CẬP NHẬT FAVICON & SEO (ĐÃ FIX: ĐƯA VÀO TRONG HÀM VÀ NÂNG CẤP) ---
  useEffect(() => {
    if (content) {
      // 1. Cập nhật SEO Title (Tiêu đề trên Tab)
      if (content.seo_title) {
        document.title = content.seo_title;
      }

      // 2. Cập nhật SEO Description
      if (content.seo_desc) {
        ...
        metaDesc.content = content.seo_desc;
      }

      // 3. Cập nhật Favicon (Xử lý triệt để hình tia sét Vite)
      if (content.favicon_image) {
        // Tìm và xóa tất cả các thẻ favicon cũ để tránh xung đột
        const existingIcons = document.querySelectorAll("link[rel*='icon']");
        existingIcons.forEach(el => el.remove());

        // Tạo thẻ link favicon mới
        const link = document.createElement('link');
        link.rel = 'icon';
        // Thêm timestamp (?v=...) để ép trình duyệt bỏ cache, luôn tải ảnh mới nhất
        link.href = `${content.favicon_image}?v=${new Date().getTime()}`;
  ...
  useEffect(() => {
    const updateMaterialsLimit = () => {
      if (window.innerWidth <= 768) setMaterialsLimit(4);
      else setMaterialsLimit(8);
    };
  ...
  useEffect(() => {
    async function loadDataFromSupabase() {
      ...
      try {
        ...
        if (galleryData) { setGalleryItems(galleryData); setFilteredItems(galleryData); }
        ...
        if (simulatorData) setSimulatorItems(simulatorData);
      } catch (err) {
        console.error("Lỗi database:", err); setMaterials(DEFAULT_MATERIALS);
      } finally {
  ...
  const handleFilter = (cat) => {
    setActiveCategory(cat); setGalleryPage(1);
    if (cat === "all") setFilteredItems(galleryItems);
    else setFilteredItems(galleryItems.filter((i) => i.category === cat));
  };

  const handleCalc = (key, val) => {
    const updated = { ...calcParams, [key]: Number(val) };
    setCalcParams(updated);
    if (updated.svc === 0) setCalcResult("Liên hệ báo giá");
    else {
      const total = Math.round(updated.svc * updated.area * updated.mat * updated.build);
  ...
  const submitContact = async (e) => {
    e.preventDefault();
    if (!contactForm.name || !contactForm.phone) { alert("Vui lòng nhập Họ tên và Số điện thoại!"); return; }
    setContactLoading(true);
    try {
      const finalNote = contactForm.note + (selectedSimulatorUrl ? " " + selectedSimulatorUrl : "");
      const { error } = await supabase.from("contacts").insert([{ ...contactForm, note: finalNote }]);
      if (error) throw error;
      setContactSuccess(true);
      setContactForm({ name: "", phone: "", email: "", service: "", area: "", address: "", note: "" });
      setSelectedSimulatorUrl("");
      setTimeout(() => setContactSuccess(false), 5000);
    } catch { alert("Có lỗi xảy ra!"); } finally { setContactLoading(false); }
  };

  const submitRecruitment = async (e) => {
    e.preventDefault();
    if (!recruitForm.name || !recruitForm.phone || !recruitForm.position) { alert("Vui lòng nhập đầy đủ!"); return; }
    try {
      const { error } = await supabase.from("recruitment").insert([recruitForm]);
      if (error) throw error;
      alert("Nộp hồ sơ thành công!");
      setShowRecruitModal(false); setRecruitForm({ name: "", phone: "", position: "", experience: "" });
    } catch { alert("Lỗi!"); }
  };
  ...
  const renderLogoText = (text) => {
    ...
    for (let i = 1; i < trimmed.length; i++) {
      if (trimmed[i] === trimmed[i].toUpperCase() && trimmed[i] !== " " && isNaN(trimmed[i])) capIndices.push(i);
    }
    ...
    if (words.length > 1) return <>{words.slice(0, words.length - 1).join(" ")} <span>{words[words.length - 1]}</span></>;
    return <>{trimmed}</>;
  };

  const getActiveSimulatorPhotos = () => {
    const matched = simulatorItems.find((item) => item.room_name === configRoom);
    if (matched && matched.image) return matched.image.split("|").filter(Boolean);
  ...
  const searchedMaterials = materials.filter((mat) => {
    ...
    return nameStr.includes(materialsSearch.toLowerCase()) || brandStr.includes(materialsSearch.toLowerCase());
  });
  ...
  const searchedReviews = reviews.filter((rev) => {
    ...
    return nameStr.includes(reviewsSearch.toLowerCase()) || textStr.includes(reviewsSearch.toLowerCase());
  });
  ...
  const handlePrevConfigPhoto = () => setConfigPhotoIdx((prev) => (prev === 0 ? activePhotosList.length - 1 : prev - 1));
  const handleNextConfigPhoto = () => setConfigPhotoIdx((prev) => (prev >= activePhotosList.length - 1 ? 0 : prev + 1));

  const handleConfiguratorSubmit = () => {
    const text = "Tôi muốn thi công theo Mẫu 3D: [" + configRoom + " - Ảnh số " + (configPhotoIdx + 1) + "].";
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => setGalleryPage((prev) => (prev >= totalGalleryPages ? 1 : prev + 1)), 10000);
    return () => clearInterval(interval);
  }, [totalGalleryPages, isGalleryHovered]);

  useEffect(() => {
    if (totalMaterialsPages <= 1 || isMaterialsHovered) return;
    const interval = setInterval(() => setMaterialsPage((prev) => (prev >= totalMaterialsPages ? 1 : prev + 1)), 10000);
    return () => clearInterval(interval);
  }, [totalMaterialsPages, isMaterialsHovered]);

  useEffect(() => {
    if (totalReviewsPages <= 1 || isReviewsHovered) return;
    const interval = setInterval(() => setReviewsPage((prev) => (prev >= totalReviewsPages ? 1 : prev + 1)), 10000);
    return () => clearInterval(interval);
